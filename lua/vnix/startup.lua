...
function M.gui_startup()
  if vnix and vnix.is_ready then
    vnix.orig_config = config.load_from_file(vnix.config_file) ---@type VnixConfig
    M.restore(vnix.orig_config)
  else
    wezterm.time.call_after(interval, M.gui_startup)
  end
end

---@param arg VnixConfig
...
function M._notify()
  ...
  if vnix.activity and vnix.activity.active_pane then
    local ap = vnix.activity.active_pane
    if ap then
  ...
  if pane then
    ...
    if gui_win then
      ...
      local pane_obj = mux.resolve_pane_by_id(pane.id)
      if pane_obj then
        pane_obj:activate()
      end
