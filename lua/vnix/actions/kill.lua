...
function M.kill_tab(win, pane, workspace, workspace_idx, tab, tab_idx)
  ...
  vnix.activity.focus[w.name] = nil
  vnix.activity.focus[w.name .. "." .. t.id] = nil
...
function M.kill_workspace(win, pane, workspace, workspace_idx)
  ...
  if #vnix.workspaces <= MIN_WORKSPACES then
  ...
  elseif wi then
    ...
    local new_workspace = vnix.workspaces[wi] or vnix.workspaces[wi + 1] or vnix.workspaces[wi - 1]
    ...
    vnix.activity.focus[w.name] = nil
    for k, _ in pairs(vnix.activity.focus) do
      if k:match("^%a+%.%d+$") then
        vnix.activity.focus[k] = nil
      end
    end
